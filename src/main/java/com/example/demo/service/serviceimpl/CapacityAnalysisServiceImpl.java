...
public class CapacityAnalysisServiceImpl implements CapacityAnalysisService {
    ...
    public CapacityAnalysisServiceImpl(TeamCapacityConfigRepository capacityRepo, EmployeeProfileRepository employeeRepo, LeaveRequestRepository leaveRepo, CapacityAlertRepository alertRepo) {
    ...
    public CapacityAnalysisResultDto analyzeTeamCapacity(String teamName, LocalDate start, LocalDate end) {
        if (start.isAfter(end)) throw new BadRequestException("Start date cannot be in the future");
        ...
        for (LocalDate date : range) {
            long onLeave = leaveRepo.findApprovedOnDate(date).stream()
                    .filter(l -> l.getEmployee().getTeamName().equals(teamName)).count();
            ...
            if (capacity < config.getMinCapacityPercent()) {
                risky = true;
                alertRepo.save(new CapacityAlert(teamName, date, "HIGH", "Capacity below threshold"));
            }
        }
